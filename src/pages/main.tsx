...
export default function Main() {
  ...
  const handleConvert = () => {
    getDolar();
    setConvertedValue(calculateConversion(typeOfPurchase, Number(dolar.replace(',', '.')), Number(taxState.replace(',', '.')), +Number(cotations.bid).toFixed(2)));
    setShowResult(true);
  };
        ...
        {!showResult ?
          <>
            ...
            <div className='auto-layout-currency-card'>
              ...
              <div className='radio-block radio-block-tipograpy'>
                <p>Compra no {typeOfPurchase === 'money' ? 'dinheiro' : 'cartão'} e taxa de {taxState.replace(',', '.')}%</p>
                <p><strong>Cotação do dólar:</strong> $1,00 = {Number(cotations.ask).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.</p>
              </div>
            </div>
          </section>
        }
